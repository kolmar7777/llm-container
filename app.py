...
@router.get("/health", summary="Check model health")
async def health():
    # Проверяем, загружена ли модель
    if _llama_proxy is None:
        return JSONResponse(status_code=503, content={"status": "service unavailable"})
    # Проверяем, заблокирован ли внешний или внутренний лок (модель занята)
    if llama_outer_lock.locked() or llama_inner_lock.locked():
        return {"status": "busy"}
    # Иначе модель свободна
    return {"status": "free"}
